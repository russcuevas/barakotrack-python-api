...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            @if($userRole === 'admin')
                ...
                <a href="{{ route('admin.claims') }}" class="nav-link {{ request()->routeIs('admin.claims') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.lost-reports') }}" class="nav-link {{ request()->routeIs('admin.lost-reports') ? 'active' : '' }}">
                    <i class="bi bi-journal-text"></i> All Lost Item Reports
                </a>

                <div class="nav-label">Actions</div>
                <a href="#" class="nav-link text-warning" data-bs-toggle="modal" data-bs-target="#reportFoundModal">
            ...
            @else
                ...
                <a href="{{ route('student.found-items') }}" class="nav-link {{ request()->routeIs('student.found-items') ? 'active' : '' }}">
                    <i class="bi bi-box-seam-fill"></i> Found Items Directory
                </a>
                <a href="{{ route('student.lost-reports') }}" class="nav-link {{ request()->routeIs('student.lost-reports') ? 'active' : '' }}">
                    <i class="bi bi-card-checklist"></i> My Reported Lost Items
                </a>
                <a href="{{ route('student.matcher') }}" class="nav-link {{ request()->routeIs('student.matcher') ? 'active' : '' }}">
                    <i class="bi bi-cpu-fill text-warning"></i> CNN AI Visual Matcher
                </a>
                <a href="{{ route('student.claims') }}" class="nav-link {{ request()->routeIs('student.claims') ? 'active' : '' }}">
                    <i class="bi bi-shield-check"></i> My Claims Tracker
                </a>

                <div class="nav-label">Actions</div>
                <a href="#" class="nav-link text-warning" data-bs-toggle="modal" data-bs-target="#reportLostModal">
        ...
        <header class="top-header d-flex justify-content-between align-items-center px-4">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="d-flex align-items-center gap-2">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($authUser->name) }}&background={{ $userRole === 'admin' ? '1e1e2d' : '752738' }}&color=fec452" class="rounded-circle" width="38" height="38" alt="User">
                    <div>
                        ...
                        <div class="text-muted fs-7" style="font-size: 0.75rem;">
                            ...
                            @else
                                <span class="badge bg-secondary">Student ({{ $authUser->student_id_number ?? 'UB-2024' }})</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <form action="{{ route('student.lost-reports.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header text-white" style="background-color: var(--primary-color);">
                        <h5 class="modal-header-title fw-bold m-0"><i class="bi bi-file-earmark-plus-fill me-2 text-warning"></i> Report Lost Item</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label fw-bold">Item Title</label>
                            <input type="text" name="title" class="form-control" placeholder="e.g. Black Sony Noise Canceling Headphones" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="date_lost" class="form-control" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">Last Known Location</label>
                                <input type="text" name="location" class="form-control" placeholder="e.g. Main Library 3rd Floor" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Detailed Description & Unique Marks</label>
                            <textarea name="description" class="form-control" rows="3" placeholder="Describe color, brand, scratches, wallpaper, contents..." required></textarea>
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary-custom px-4"><i class="bi bi-send-fill me-1"></i> Submit Report</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.inventory.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header text-white" style="background-color: #1e1e2d;">
                        <h5 class="modal-header-title fw-bold m-0"><i class="bi bi-box-arrow-in-down me-2 text-warning"></i> Register Surrendered Found Item</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label fw-bold">Item Title</label>
                            <input type="text" name="title" class="form-control" placeholder="e.g. Brown Leather Wallet" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Category</label>
                            <select name="category_id" class="form-select" required>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="date_found" class="form-control" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">Location Found</label>
                                <input type="text" name="location" class="form-control" placeholder="e.g. Student Center Cafeteria" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">SAO Storage Location / Cabinet #</label>
                            <input type="text" name="storage_location" class="form-control" placeholder="e.g. SAO Office Cabinet B1 / Safe #2" required>
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary-custom px-4"><i class="bi bi-box-seam me-1"></i> Register into Inventory</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('student.claims.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header text-white" style="background-color: var(--primary-color);">
                        <h5 class="modal-header-title fw-bold m-0"><i class="bi bi-shield-check me-2 text-warning"></i> Submit Claim Request</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="proof_description" class="form-control" rows="4" placeholder="Describe unique features non-publicly known (e.g., lock code, wallpaper, name sticker, receipt number, internal contents)..." required></textarea>
                        </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-secondary-custom px-4 fw-bold"><i class="bi bi-check-circle-fill me-1"></i> Submit Claim to SAO</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="chatbot-widget">
        <div class="chatbot-box hidden" id="chatbotBox">
            ...
            <div class="p-2 bg-light border-top d-flex gap-1 overflow-auto">
                <div class="suggestion-chip" onclick="sendChatQuery('How to report a lost item?')">Report Lost</div>
                <div class="suggestion-chip" onclick="sendChatQuery('Where is the lost and found office?')">SAO Office</div>
    ...
    <script>
    function openClaimModal(itemId, itemTitle, storageLoc) {
    ...
    async function sendChat() {
        ...
        try {
            ...
            const botMsg = document.createElement('div');
            botMsg.className = 'chat-bubble bot';
            botMsg.innerHTML = "I am Brahmmy! You can report lost items online, view found items, or visit SAO (Mon-Fri 8AM-5PM).";
            chatBox.appendChild(botMsg);
        }
        chatBox.scrollTop = chatBox.scrollHeight;
    }
