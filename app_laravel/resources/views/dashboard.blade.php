...
@section('title', $activeRole === 'admin' ? 'Barako Track | SAO Admin Control Panel' : 'Barako Track | Student Dashboard')
...
@if($activeRole === 'student')
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-3">
            <div class="card card-custom stat-card p-3">
                ...
                <h2 class="fw-bold my-1" style="color: var(--primary-color);">{{ $studentClaims->where('status', 'pending')->count() }}</h2>
    ...
    <div class="card card-custom p-4 mb-4" id="my-reports">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="fw-bold m-0" style="color: var(--primary-color);"><i class="bi bi-card-checklist me-2"></i> My Reported Lost Items</h5>
            <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#reportLostModal">+ Report Lost</button>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle m-0">
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-3">You have not submitted any lost item reports.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="card card-custom p-4 mb-4" id="my-claims">
        <h5 class="fw-bold mb-3" style="color: var(--primary-color);"><i class="bi bi-shield-check me-2 text-warning"></i> My Claims Tracker</h5>
        <div class="table-responsive">
            <table class="table table-hover align-middle m-0">
                <thead class="table-light">
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="fw-bold">{{ $claim->foundItem ? $claim->foundItem->title : 'Item #' . $claim->found_item_id }}</div>
                            <small class="text-muted"><i class="bi bi-building"></i> {{ $claim->foundItem->storage_location ?? 'SAO Storage' }}</small>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-3">No claims submitted yet. Browse the Found Items directory above to claim an item.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
@else
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <span class="text-muted">Manage campus found inventory, verify student proof of ownership claims, and track reports.</span>
        </div>
    ...
    <div class="card card-custom p-4 mb-4 border-warning shadow-sm" id="pending-claims">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="fw-bold m-0 text-dark"><i class="bi bi-shield-exclamation text-warning me-2"></i> Pending Student Claims Needing Verification</h5>
            <span class="badge bg-warning text-dark">{{ $allClaims->where('status', 'pending')->count() }} Pending</span>
        </div>
        <div class="table-responsive">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted">ID: {{ $pendingClaim->user->student_id_number ?? 'UB-Student' }}</small><br>
                            <small class="text-muted"><i class="bi bi-telephone"></i> {{ $pendingClaim->user->phone ?? 'N/A' }}</small>
                        </td>
                        <td>
                            <div class="fw-bold text-primary">{{ $pendingClaim->foundItem->title ?? 'Found Item' }}</div>
                            <small class="text-muted"><i class="bi bi-geo-alt"></i> Found: {{ $pendingClaim->foundItem->location ?? 'Campus' }}</small><br>
                            <span class="badge bg-secondary"><i class="bi bi-building"></i> {{ $pendingClaim->foundItem->storage_location ?? 'SAO' }}</span>
                        </td>
                        <td style="max-width: 280px;">
                            <p class="fs-7 mb-2 bg-light p-2 rounded border">{{ $pendingClaim->proof_description }}</p>
                            @if($pendingClaim->proof_image)
                                <a href="{{ $pendingClaim->proof_image }}" target="_blank" class="btn btn-xs btn-outline-primary py-0 px-2 fs-7">
                        ...
                        <td style="width: 260px;">
                            <!-- Approve Form -->
                            <form action="{{ route('admin.claims.approve', $pendingClaim->id) }}" method="POST" class="mb-2">
                                @csrf
                                <div class="input-group input-group-sm mb-1">
                                    <input type="text" name="admin_notes" class="form-control" placeholder="Admin verification note..." value="Proof verified by SAO Staff. Ready for pickup.">
                                </div>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted py-4"><i class="bi bi-check2-all text-success me-1"></i> No pending claims requiring verification. Great job!</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="card card-custom p-4 mb-4" id="inventory-management">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="fw-bold m-0" style="color: var(--primary-color);"><i class="bi bi-archive-fill me-2"></i> SAO Storage Inventory Directory</h5>
            <button class="btn btn-sm btn-primary-custom" data-bs-toggle="modal" data-bs-target="#reportFoundModal">+ Add Found Item</button>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle m-0">
                <thead class="table-light">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted">Found at {{ $fItem->location }} on {{ $fItem->date_found->format('M d, Y') }}</small>
                        </td>
                        <td><span class="badge bg-light text-dark border">{{ $fItem->category->name }}</span></td>
                        <td><span class="badge bg-info text-dark"><i class="bi bi-box-seam"></i> {{ $fItem->storage_location }}</span></td>
                        <td>
                        ...
                        <td>
                            <form action="{{ route('found-items.update-status', $fItem->id) }}" method="POST" class="d-flex gap-1">
                                @csrf
                                @method('PUT')
                                <select name="status" class="form-select form-select-sm" onchange="this.form.submit()">
                                    <option value="available" {{ $fItem->status === 'available' ? 'selected' : '' }}>Available</option>
                                    <option value="claim_pending" {{ $fItem->status === 'claim_pending' ? 'selected' : '' }}>Claim Pending</option>
                                    <option value="claimed" {{ $fItem->status === 'claimed' ? 'selected' : '' }}>Claimed</option>
                                    <option value="disposed" {{ $fItem->status === 'disposed' ? 'selected' : '' }}>Disposed</option>
                                </select>
                            </form>
                        </td>
                    </tr>
    ...
    <div class="card card-custom p-4 mb-4" id="all-reports">
        <h5 class="fw-bold mb-3" style="color: var(--primary-color);"><i class="bi bi-journal-text me-2"></i> All Campus Lost Item Reports</h5>
        <div class="table-responsive">
            <table class="table table-hover align-middle m-0">
                <thead class="table-light">
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted"><i class="bi bi-geo-alt"></i> {{ $lReport->location }}</small>
                        </td>
                        <td>
